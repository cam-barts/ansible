...
| --- | --- | --- |
| `configure_borgmatic.yml` | citadel + warrig (narrow via `-e borgmatic_target=`), delegating to gastown | Installs borgmatic, renders config + hook scripts, generates each host's SSH key and authorizes it on gastown, and schedules the nightly pushes (citadel 03:00, warrig 04:00). Per-host values live in host_vars. See `roles/borgmatic/README.md`. |
