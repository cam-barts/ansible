...
defaults to the public `https://ntfy.sh` — meaning any host without config
leaks notifications onto a public topic.

## What it does NOT do

- Install the `ntfy` CLI itself — that's platform-specific. On Arch:
...
- `borgmatic_citadel` — declares dependency in `meta/main.yml`
- `rsnapshot_gastown` — should declare dependency once gastown is converted
  to use the system-wide config instead of the wrapper-embedded path
- Any future role/host that calls `ntfy` from a script or systemd unit
